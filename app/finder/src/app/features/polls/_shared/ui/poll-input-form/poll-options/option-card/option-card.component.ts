...
export class OptionCardComponent {
  ...
  private descriptionInput =
    viewChild.required<DsTextareaComponent>('descriptionInput');
  ...
  onLinkUrlChange(value: string) {
    this.optionChange.emit({
      ...this.option(),
      meta: { ...this.option().meta, url: value },
    });
  ...
  onTitleBlur() {
    ...
    if (!text || !this.urlValidation.isValid(text) || this.urlLoading()) {
      return;
    }
    ...
    this.previewService
      ...
      .subscribe({
        next: (preview) => {
          ...
          if (preview.title) {
            updatedEntry.text = preview.title.substring(
              0,
              POLL_LIMITS.optionTextLength,
            );
          }
          if (preview.description && !entry.description) {
            updatedEntry.description = preview.description.substring(
              0,
              POLL_LIMITS.optionDescriptionLength,
            );
            this.showDescription.set(true);
          }
  ...
  onUrlBlur() {
    ...
    if (normalized !== url) {
      this.optionChange.emit({
        ...this.option(),
        meta: { ...this.option().meta!, url: normalized },
      });
      this._linkUrl.set(normalized);
    }
      ...
      .subscribe({
        next: (preview) => {
          if (preview.imageUrl) {
            ...
            const updatedEntry: OptionEntry = {
              ...entry,
              meta: { url: entry.meta!.url, ...preview },
            };
            if (!entry.text) {
              updatedEntry.text = (preview.title ?? '').substring(
                0,
