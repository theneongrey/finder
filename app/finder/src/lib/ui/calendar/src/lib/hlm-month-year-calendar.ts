        ...
        <div class="flex flex-col gap-4">
            ...
            @switch (_picker.view()) {
                @case ('year') {
                    ...
                    >
                        @for (
                            year of _picker.years();
                            track _dateAdapter.getYear(year)
                        ) {
                            <button
                            ...
                            >
                                {{
                                    _i18n
                                        .config()
                                        .formatYear(_dateAdapter.getYear(year))
                                }}
                            </button>
                        }
                    </div>
                ...
                @case ('month') {
                    ...
                    >
                        @for (
                            month of _picker.months();
                            track _dateAdapter.getMonth(month)
                        ) {
                            <button
...
export class HlmMonthYearCalendar<T> {
    ...
    protected readonly _heading = computed(() => {
        ...
        if (this._picker.view() === 'month') {
            return config.formatYear(
                this._dateAdapter.getYear(this._picker.focusedDate()),
            );
        }
