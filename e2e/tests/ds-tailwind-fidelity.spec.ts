...
test.describe('ds-* Tailwind migration — visual fidelity (/ux)', () => {
  ...
  test('ds-badge: every tone renders a visible background', async ({ page }) => {
  ...
  test('ds-chip: active and inactive states differ and toggle', async ({
    ...
    await expect.poll(() => css(inactive, 'background-color')).toBe(activeBg);
  });
  ...
  test('ds-vote-buttons: maybe is smaller than yes and white with a border', async ({
    ...
    const yesBox = await yes.boundingBox();
    expect(maybeBox, 'maybe button box').not.toBeNull();
    expect(yesBox, 'yes button box').not.toBeNull();
  ...
  test('ds-poll-card-skeleton: shimmer keyframes still animate', async ({
    ...
    expect(await css(shimmer, 'animation-name')).toContain('ds-skeleton-shimmer');
  });
  ...
  test('ds-bottom-sheet: bottom sheet on narrow viewport, centred modal on wide', async ({
    ...
    await expect(panel).toBeVisible();
    // Poll until the open animation settles at the horizontal centre (~500px),
    // then read the settled box for the remaining geometry assertions.
    await expect
      .poll(async () => {
