...
**Global styles bleed**
- Open `app/finder/src/styles.css` (or `global_styles.css`). For each class added there, check if it is used in more than one component. If it's only used in one component, flag it as a candidate to move to that component's `.component.css`.
...
- Hex colours or pixel values that appear more than once in the same file and could be CSS custom properties
- Colours that look like they should come from design tokens (`var(--accent)`, `var(--text-primary)`, etc.) but are hardcoded

**Budget**
- If `angular.json` `anyComponentStyle` warning/error thresholds are raised, flag this as an altitude problem — the right fix is splitting the CSS, not raising the budget
