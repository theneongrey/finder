...
function applyCalendarLocale(lang: string, calendarI18n: ReturnType<typeof injectBrnCalendarI18n>): void {
  calendarI18n.use({
    months: () =>
      Array.from({ length: 12 }, (_, i) =>
        new Intl.DateTimeFormat(lang, { month: 'short' }).format(new Date(2024, i)),
    ...
    formatWeekdayName: (index) =>
      new Intl.DateTimeFormat(lang, { weekday: 'short' }).format(new Date(2024, 0, 7 + index)),
    labelWeekday: (index) =>
      new Intl.DateTimeFormat(lang, { weekday: 'long' }).format(new Date(2024, 0, 7 + index)),
  });
}
...
export class AppComponent {
  constructor() {
    ...
    translateService.onLangChange.subscribe(({ lang }) => applyCalendarLocale(lang, calendarI18n));
  }
}
